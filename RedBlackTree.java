...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    if (child.context[0] == null || !child.context[0].equals(parent)) {
      throw new IllegalArgumentException(
          "\"Child\" node and \"parent node\" are not actually" + " related!");
    } else if (!child.isRightChild()) {
      // right rot
      parent.context[1] = child.context[2];// set parent's left subtree to child's right
      if (child.context[2] != null) {
        child.context[2].context[0] = parent;// set child's right subtree's parent to parent
      }
      if (parent.equals(root)) {
        root = child;// set the root to the child
      } else {
        if (!parent.isRightChild()) {
          parent.context[0].context[1] = child;// set grandparent's left subtree to child
          child.context[0] = parent.context[0];// set child's parent to grandparent
        } else {
          parent.context[0].context[2] = child;// set grandparent's right subtree to child
          child.context[0] = parent.context[0];// set child's parent to grandparent
        }
      }
      child.context[2] = parent;// set child's right subtree to parent
      parent.context[0] = child;// set parent's parent to child
    } else {
      // left rot
      parent.context[2] = child.context[1];// set parent's right subtree to child's right
      if (child.context[1] != null) {
        child.context[1].context[0] = parent;// set child's left subtree's parent to parent
      }
      if (parent.equals(root)) {
        root = child;// set the root to the child
      } else {
      ...
      child.context[1] = parent;// set child's left subtree to parent
      parent.context[0] = child;// set parent's parent to child
    }
  }
  ...
  protected void enforceRBTreePropertiesAfterInsert(Node<T> node) {
    // set a relational node: this way I can access it later much easier
    Node<T> relation = null;
    // while statement: while the node isn't the root, while the grandparent isn't null, and while
    // the parent is red
    while (node != this.root && node.context[0].context[0] != null
        && node.context[0].blackHeight == 0) {
      // set the parent so easier access
      Node<T> parent = node.context[0];
      // if the parent is the left child...
      if (parent == parent.context[0].context[1]) {
        // set the relation to the sibling of the parent...
        relation = parent.context[0].context[2];
        // then check if the relation is null or black...
        if (relation == null || relation.blackHeight == 1) {
          // if it is, then check if the node is the right child...
          if (node == parent.context[2]) {
            // node becomes parent, and then rotate the right child and the parent to the right
            // positions
            node = parent;
            rotate(node.context[2], node);
          }
          // rotate the node and the grandparent
          ...
          // set the relation and parent to be black
          relation.blackHeight = 1;
          parent.blackHeight = 1;
          // if not root...
          if (parent.context[0] != this.root) {
            // set the grandparent to red
            parent.context[0].blackHeight = 0;
          }
          // set the node to the grandparent, and repeat
          node = parent.context[0];
        }
      } else if (parent == parent.context[0].context[2]) {
        // set the relation to the sibling of the parent...
        relation = parent.context[0].context[1];
        // then check if the relation is null or black...
        if (relation == null || relation.blackHeight == 1) {
          // if it is, then check if the node is the right child...
    ...
    // set the root to black regardless of what happens
